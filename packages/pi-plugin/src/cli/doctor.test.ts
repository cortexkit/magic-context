...
import type { PiDiagnosticReport } from "./diagnostics";
import { type RunDoctorOptions, runDoctor } from "./doctor";
...
class MockPrompts implements PromptIO {
	...
	async selectOne(_message: string, options: SelectOption[]): Promise<string> {
		return (
			options.find((option) => option.recommended)?.value ?? options[0].value
		);
	}
}
...
function baseOptions(
	...
	return {
		...
		deps: {
			...
			spawnSync: () =>
				({ status: 1, stdout: "", stderr: "not expected" }) as never,
		},
	};
}
