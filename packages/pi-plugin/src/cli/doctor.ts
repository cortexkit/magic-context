...
import { type PromptIO, promptIO } from "./prompts";
...
function printDoctorHelp(): void {
	...
	console.log(
		"    magic-context-pi doctor --force  Repair safe issues, then re-check",
	);
	console.log(
		"    magic-context-pi doctor --issue  Create a sanitized bug report",
	);
...
function summarize(
	results: CheckResult[],
): Pick<HealthReport, "pass" | "warn" | "fail"> {
	return {
...
async function runHealthChecks(options: {
	...
	if (latest && compareSemver(self, latest) === -1) {
		...
		add(
			results,
			"info",
			`Magic Context for Pi CLI v${self}; npm latest check unavailable`,
		);
	}
	...
	if (!existsSync(settingsPath)) {
		...
		if (parsed.error) {
			add(
				results,
				"fail",
				`Could not parse Pi settings ${settingsPath}: ${parsed.error}`,
			);
		} else {
			...
			if (hasPiPackage(packages)) {
				add(
					results,
					"pass",
					`${PI_PACKAGE_SOURCE} is registered in packages[]`,
				);
			} else {
	...
	for (const [label, path, required] of [
		...
		if (!existsSync(path)) {
			if (required) {
				add(
					results,
					"warn",
					`No ${label} magic-context.jsonc found at ${path}`,
				);
				...
				add(
					results,
					"info",
					`No project Pi magic-context.jsonc found at ${path}`,
				);
			}
			continue;
		}
		...
		if (parsed.error)
			add(
				results,
				"fail",
				`${label} magic-context.jsonc is invalid JSONC: ${parsed.error}`,
			);
		else
			add(
				results,
				"pass",
				`${label} magic-context.jsonc is valid JSONC: ${path}`,
			);
	}

	const loadedConfig = loadPiConfig({ cwd: options.cwd });
	if (loadedConfig.warnings.length > 0) {
		for (const warning of loadedConfig.warnings.slice(0, 4))
			add(results, "warn", warning);
	...
	if (existedBeforeOpen)
		add(results, "pass", `Shared context DB exists at ${dbPath}`);
	else
		add(
			results,
			"warn",
	...
	try {
		...
		const counts = ROW_COUNT_TABLES.map(
			(table) =>
				`${table}=${countTable(db as ContextDatabase, table) ?? "n/a"}`,
		).join(", ");
	...
	if (mergedEmbedding.provider === "openai-compatible") {
		const endpoint =
			typeof mergedEmbedding.endpoint === "string"
				? mergedEmbedding.endpoint.trim()
				: "";
		const model =
			typeof mergedEmbedding.model === "string"
				? mergedEmbedding.model.trim()
				: "";
		const apiKey =
			typeof mergedEmbedding.api_key === "string"
				? mergedEmbedding.api_key
				: undefined;
		if (!endpoint || !model) {
			add(
				results,
		...
		add(
			results,
			"pass",
			`Embedding provider: ${loadedConfig.config.embedding.provider}`,
		);
	}
	...
	if (otherExtensions.length > 0) {
		add(
			results,
			"info",
			`Other Pi extensions registered: ${otherExtensions.join(", ")}`,
		);
	} else {
	...
	if (staleCaches.length > 0) {
		...
		add(
			results,
			"pass",
			"Pi extension cache clean (no stale cached package found)",
		);
	}
	...
	if (existsSync(logPath)) {
		...
		add(
			results,
			"info",
			`Last plugin log line: ${lines.at(-1) ?? "<empty log>"}`,
		);
	} else {
	...
	if (plan.writeUserConfig) {
		...
		if (!existsSync(configPath)) {
			try {
				...
				prompts.log.success(
					`Wrote default Magic Context config to ${configPath}`,
				);
				fixed += 1;
			} catch (error) {
				console.error(
		...
		if (ghAvailableAndAuthed(options.deps)) {
			...
			if (shouldSubmit) {
				...
				options.prompts.log.warn(
					String(result.stderr).trim() || "gh issue create failed",
				);
			}
		} else {
			options.prompts.log.warn(
...
export async function runDoctor(
	...
	console.log("");
	prompts.log.message(
		`Summary: PASS ${first.pass} / WARN ${first.warn} / FAIL ${first.fail}`,
	);
	...
	if (options.force) {
		...
		prompts.outro(
			second.fail > 0
				? "Doctor found failures after repair"
				: "Doctor repair complete",
		);
