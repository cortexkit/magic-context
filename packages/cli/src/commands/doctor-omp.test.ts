...
describe("OMP doctor", () => {
    it("accepts a healthy OMP installation", async () => {
        ...
        // Must REDIRECT, never delete: bun caches os.homedir() at startup, so
        // process.env.HOME above cannot move getDataDir()'s fallback. Deleting
        // XDG_DATA_HOME therefore escapes test-preload.ts isolation and points
        // the doctor's integrity_check at the user's real multi-GB context.db.
