...
describe("createTransform", () => {
    ...
    it("does not run caveman text compression for an opted-in child", async () => {
        useTempDataHome("context-transform-caveman-child-");
        const sessionId = "ses-caveman-opted-child";
        const db = openDatabase();
        updateSessionMeta(db, sessionId, { isSubagent: true });
        const transform = createTransform({
            tagger: createTagger(),
        ...
        const original =
            "I just wanted to basically clearly explain this detailed child context. ".repeat(5);
        const messages: TestMessage[] = [
            {
                info: {
                    id: "m-child",
        ...
        expect(text(messages[1], 0)).toBe(`§2§ ${original}`);
