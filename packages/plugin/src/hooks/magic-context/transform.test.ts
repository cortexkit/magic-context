...
describe("createTransform", () => {
	it("logs both nudge gate verdicts on every evaluable TypeScript pass", async () => {
		...
		try {
			...
			const channel1 = lines.filter((line) =>
				line.startsWith("channel1 evaluation:"),
			);
			const channel2 = lines.filter((line) =>
				line.startsWith("channel2 evaluation:"),
			);
			...
			expect(channel1.every((line) => line.includes("growth_threshold="))).toBe(
				true,
			);
			expect(
				channel1.every((line) =>
					line.includes("sticky_floor_turns_remaining="),
				),
			).toBe(true);
	...
	it("preserves the pre-optimization served-wire digest for mixed replay states", async () => {
		...
		const input: TestMessage[] = [
			...
			},
			{
				...
				parts: [
					{ type: "text", text: "Previously compacted message still visible." },
				],
			},
			{
		...
		for (const mode of ["full", "truncated", "edit_marker"] as const) {
			const tag = tags.find(
				(tag) => tag.type === "tool" && tag.messageId === `call-${mode}`,
			);
		...
		expect(served.some((message) => message.info.id === "tool-full")).toBe(
			false,
		);
		for (const mode of ["truncated", "edit_marker"]) {
			const message = served.find(
				(message) => message.info.id === `tool-${mode}`,
			);
			expect(message).toBeDefined();
			expect(toolOutput(message!, 0)).toContain("[dropped");
		}
		const compressed = served.find(
			(message) => message.info.id === "compressed",
		);
		expect(compressed).toBeDefined();
		expect(text(compressed!, 0)).not.toContain("I just wanted");
		expect(text(compressed!, 0).length).toBeGreaterThan(0);
		const digest = createHash("sha256")
			.update(JSON.stringify(served))
			.digest("hex");
		...
		expect(digest).toBe(
			"0e3b74f2fc4379eeb93244788f0eb21920ac7892570303858c64cd2a19d810b0",
		);
	});
	...
	it("serves byte-identical hot passes while coalescing state and skipping all-hit token SQL", async () => {
		...
		expect(
			preparedSql.filter((sql) =>
				sql.includes("SELECT historian_failure_count"),
			).length,
		).toBe(1);
		expect(
			preparedSql.filter((sql) =>
				sql.includes("SELECT stale_reduce_stripped_ids"),
			).length,
		).toBe(1);
		expect(
			preparedSql.some((sql) => sql.includes("SELECT last_response_time FROM")),
		).toBe(false);
		expect(
			preparedSql.some(
				(sql) =>
	...
	it("keeps the raw array untouched when session metadata is unreadable", async () => {
		...
		const transform = createTransform({
			...
			contextUsageMap: new Map<
				string,
				{ usage: ContextUsage; updatedAt: number }
			>(),
			db,
		...
		const transform = createTransform({
			...
			contextUsageMap: new Map([
				[
					sessionId,
					{
						usage: { percentage: 40, inputTokens: 40_000 },
						updatedAt: Date.now(),
					},
				],
			]),
	...
	it("captures distinct LKG prefixes and token telemetry consumed by sidebar snapshots", async () => {
		...
		expect(getSlot(shortSession)?.jsonPrefix).not.toBe(
			getSlot(longSession)?.jsonPrefix,
		);
		...
		expect(shortSnapshot.conversationTokens).not.toBe(
			toolSnapshot.conversationTokens,
		);
	...
	it("tags text/tool content without injecting any assistant-anchored nudge", async () => {
		...
		>([
			[
				"ses-1",
				{
					usage: { percentage: 46, inputTokens: 92_000 },
					updatedAt: Date.now(),
				},
			],
		]);
		...
		const transform = createTransform({
			...
			>([
				[
					"ses-no-user-nudge",
					{
						usage: { percentage: 81, inputTokens: 162_000 },
						updatedAt: Date.now(),
					},
				],
			]),
		...
		const transform = createTransform({
			...
			>([
				[
					"ses-compartment-skip",
					{
						usage: { percentage: 30, inputTokens: 60_000 },
						updatedAt: Date.now(),
					},
				],
			]),
	...
	for (const historianRunnable of [true, false]) {
		it(`replays stored child history while reconciliation is off without starting new historian work (historian runnable=${historianRunnable})`, async () => {
			useTempDataHome("context-transform-subagent-history-replay-");
			const sessionId = "ses-subagent-history-replay";
			const db = openDatabase();
			...
			replaceAllCompartments(db, sessionId, [
				{
					...
					title: "Child summary",
					content: "published child history",
				},
			]);
			const runHistorian = mock(async () => {
				throw new Error("disabled reconciliation must not invoke historian");
			});
			const transform = createTransform({
				tagger: createTagger(),
				scheduler: { shouldExecute: () => "execute" },
				contextUsageMap: new Map(),
				db,
				historyRefreshSessions: new Set(),
				pendingMaterializationSessions: new Set(),
				...
				clearReasoningAge: 50,
				protectedTokens: 0,
				hiddenCompletionExecutor: { runHistorian } as never,
				historianRunnable,
				subagentReconciliation: false,
			});
			const messages: TestMessage[] = [
				{
					info: { id: "covered", role: "user", sessionID: sessionId },
					parts: [{ type: "text", text: "old child turn" }],
				},
				{
					info: { id: "tail", role: "user", sessionID: sessionId },
			...
			expect(runHistorian).not.toHaveBeenCalled();
			expect(messages).toHaveLength(1);
			expect(messages[0]?.info.id).toBe("tail");
			expect(text(messages[0]!, 0)).toContain("<session-history>");
			expect(text(messages[0]!, 0)).toContain("published child history");
		});
	}

	it("does not trigger a disabled historian for an opted-in child under pressure", async () => {
		useTempDataHome("context-transform-subagent-disabled-historian-");
		const sessionId = "ses-subagent-disabled-historian";
		const db = openDatabase();
		updateSessionMeta(db, sessionId, { isSubagent: true });
		const runHistorian = mock(async () => {
			throw new Error("historian.disable must prevent child reconciliation");
		});
		const transform = createTransform({
			tagger: createTagger(),
			scheduler: { shouldExecute: () => "execute" },
			contextUsageMap: new Map([
				[
					sessionId,
					{
						usage: { percentage: 90, inputTokens: 180_000 },
						updatedAt: Date.now(),
					},
				],
			]),
		...
		const messages: TestMessage[] = [
			{
				info: { id: "child-pressure", role: "user", sessionID: sessionId },
				parts: [{ type: "text", text: "high pressure child request" }],
			},
		];
	...
	it("leaves an internal child first pass untouched even when reconciliation is enabled", async () => {
		useTempDataHome("context-transform-internal-child-first-pass-");
		const sessionId = "ses-internal-child-first-pass";
		const transform = createTransform({
			tagger: createTagger(),
			scheduler: { shouldExecute: () => "execute" },
			contextUsageMap: new Map(),
			...
			protectedTokens: 0,
			subagentReconciliation: true,
			internalChildSessions: new Set([sessionId]),
		});
		const messages: TestMessage[] = [
			{
				info: { id: "internal", role: "user", sessionID: sessionId },
				parts: [{ type: "text", text: "fixed internal prompt" }],
			},
		];
	...
	it("keeps compartment history visible when the first uncovered message is already dropped", async () => {
		...
		const baselineTransform = createTransform({
			...
			>([
				[
					...
					{
						usage: { percentage: 25, inputTokens: 50_000 },
						updatedAt: Date.now(),
					},
				],
			]),
		...
		const droppedCarrierTag = getTagsBySession(
			db,
			"ses-compartment-dropped-carrier",
		).find((tag) => tag.messageId === "m-4:p0");
		...
		const flushedMaterialization = new Set<string>([
			"ses-compartment-dropped-carrier",
		]);
	...
	it("creates a synthetic history carrier when all visible messages are already covered", async () => {
		...
		const transform = createTransform({
			...
			>([
				[
					...
					{
						usage: { percentage: 20, inputTokens: 40_000 },
						updatedAt: Date.now(),
					},
				],
			]),
		...
		const messages: TestMessage[] = [
			{
				info: {
					id: "m-1",
					role: "user",
					sessionID: "ses-compartment-all-covered",
				},
				parts: [{ type: "text", text: "old 1" }],
			},
			{
		...
		expect(messages[0]?.info.id).toBeUndefined();
		expect(text(messages[0]!, 0)).toContain("<session-history>");
		expect(text(messages[0]!, 0)).toContain(
			"Everything is already summarized.",
		);
	});
	...
	it("yields the protected token window at absolute emergency pressure", async () => {
		...
		const bigOutput =
			"const value = compute(input, options); // step output line\n".repeat(
				3400,
			);
		const transform = createTransform({
			tagger: createTagger(),
			...
			>([
				[
					...
					{
						usage: { percentage: 96, inputTokens: 192_000 },
						updatedAt: Date.now(),
					},
				],
			]),
		...
		const messages: TestMessage[] = [
			{
				info: {
					id: "m-user",
					role: "user",
					sessionID: "ses-force-materialize",
				},
	...
	it("keeps step-start native for github-copilot but sentinelizes it for anthropic", async () => {
		...
		//#when
		const copilotMessages = await runForProvider(
			"github-copilot",
			"ses-copilot-step-start",
		);
		const anthropicMessages = await runForProvider(
			"anthropic",
			"ses-anthropic-step-start",
		);

		//#then
		expect(copilotMessages[1].parts[1]).toEqual({
			type: "step-start",
			text: "start",
		});
	...
	it("produces byte-identical structural output on cold DB-recovered and hot anthropic passes", async () => {
		...
		//#then
		expect(JSON.stringify(hotMessages[1].parts)).toBe(
			JSON.stringify(coldMessages[1].parts),
		);
	});
	...
	it("uses the DB-recovered provider for both main transform and postprocess in one anthropic pass", async () => {
		...
		const scheduler: Scheduler = {
			shouldExecute: mock(() => "execute" as const),
		};
		const transform = createTransform({
			tagger: createTagger(),
			scheduler,
			...
			>([
				[
					sessionId,
					{
						usage: { percentage: 60, inputTokens: 120_000 },
						updatedAt: Date.now(),
					},
				],
			]),
	...
	it("applies pending drop operations when scheduler executes", async () => {
		...
		expect(channel1StateBySession.get("ses-1")?.agentDropsAppliedThisPass).toBe(
			true,
		);
		...
		const firstPass: TestMessage[] = [
			...
			},
			{
				...
				parts: [
					{
						type: "text",
						text: "long assistant response that should be dropped",
					},
				],
			},
		];
		...
		const transform = createTransform({
			...
			>([
				[
					"ses-m0m1-first",
					{
						usage: { percentage: 10, inputTokens: 1_000 },
						updatedAt: Date.now(),
					},
				],
			]),
		...
		const transform = createTransform({
			...
			>([
				[
					"ses-sub-drop",
					{
						usage: { percentage: 52, inputTokens: 104_000 },
						updatedAt: Date.now(),
					},
				],
			]),
	...
	it("yields the protected token window for subagents at >=95%", async () => {
		...
		const messages: TestMessage[] = [
			...
			},
			{
				...
				parts: [
					{ type: "tool", callID: "call-2", state: { output: bigOutput } },
				],
			},
		];
		await transform({}, { messages });
		...
		const transform = createTransform({
			...
			>([
				[
					"ses-sub-ch1",
					{
						usage: { percentage: 50, inputTokens: 100_000 },
						updatedAt: Date.now(),
					},
				],
			]),
	...
	it("runs caveman compression for a primary with ctx_reduce available while skipping dropped tags", async () => {
		...
		const transform = createTransform({
			...
			>([
				[
					sessionId,
					{
						usage: { percentage: 70, inputTokens: 100_000 },
						updatedAt: Date.now(),
					},
				],
			]),
		...
		expect(
			getTagsBySession(db, sessionId).some((tag) => tag.cavemanDepth > 0),
		).toBe(true);
	});
	...
	it("preserves once-per-turn guard for primary sessions (does NOT re-run heuristics within one turn)", async () => {
		...
		const firstPrimaryToolTag = primaryTags.find(
			(t) => t.messageId === "call-1",
		);
		expect(firstPrimaryToolTag?.status).toBe("active");
		const secondPrimaryToolTag = primaryTags.find(
			(t) => t.messageId === "call-2",
		);
		...
		const messages: TestMessage[] = [
			{
				...
				parts: [
					{ type: "text", text: `${injectedPrefix}original user message` },
				],
			},
		];
		...
		const transform = createTransform({
			...
			>([
				[
					"ses-tagger-fail",
					{
						usage: { percentage: 40, inputTokens: 80_000 },
						updatedAt: Date.now(),
					},
				],
			]),
		...
		const transform = createTransform({
			...
			>([
				[
					"ses-scheduler-fail",
					{
						usage: { percentage: 66, inputTokens: 132_000 },
						updatedAt: Date.now(),
					},
				],
			]),
	...
	it("applies persisted drops even without contextUsageMap entry or persisted usage", async () => {
		...
		if (toolTag)
			updateTagStatus(db, "ses-no-usage", toolTag.tagNumber, "dropped");
...
describe("createTransform protected tail", () => {
	...
	it("clears stale compartmentInProgress when no eligible history exists", async () => {
		...
		expect(
			getOrCreateSessionMeta(db, "ses-pt-pending").compartmentInProgress,
		).toBe(true);
...
describe("createTransform shrinking model-switch overflow pre-arm", () => {
				...
				providers: async () => ({
					data: {
						providers: [
							{
								id: "openai",
								models: { "gpt-5.5": { limit: { input: inputCap } } },
							},
						],
					},
				}),
	...
	it("does not abort a stale proactive arm after restart zeroed the input sample", async () => {
		...
		createOpenCodeDbForTransform(sessionId, [
			{ id: "m-raw-1", role: "user", text: "recent" },
		]);
		const db = openDatabase();
		recordOverflowDetected(
			db,
	...
	it("does not arm for a subagent session", async () => {
		...
		expect(getOverflowState(db, "ses-shrink-sub").needsEmergencyRecovery).toBe(
			false,
		);
	});
	...
	it("still arms when the newest user message lacks info.model (falls back to the live map, not the OLD assistant)", async () => {
		...
		expect(getOverflowState(db, "ses-nomodel").needsEmergencyRecovery).toBe(
			true,
		);
	});
});
...
describe("createTransform historian failure handling", () => {
	it("fails closed until the bounded no-head escape lowers synthetic pressure", async () => {
		...
		expect(
			loadProtectedTailMeta(db, "ses-empty-head-escape")
	...
	it("keeps failing closed at real >=95% pressure after the no-head escape", async () => {
		...
		const transform = createTransform({
			...
			contextUsageMap: new Map([
				[
					"ses-empty-head-escape-95",
					{
						usage: { percentage: 96, inputTokens: 7_680 },
						updatedAt: Date.now(),
					},
				],
			]),
		...
		expect(
			loadProtectedTailMeta(db, "ses-empty-head-escape-95")
				.recoveryNoEligibleHeadCount,
		).toBe(2);
	});
	...
	it("escalates a latched force-pressure episode to provider-overflow fail-closed recovery", async () => {
		...
		const transform = createTransform({
			...
			client: {
				session: {
					get: mock(async () => ({
						data: { directory: "/tmp", title: "Episode" },
					})),
					prompt,
					abort,
				},
	...
	it("notifies before awaiting self-abort for provider-proven overflow", async () => {
		...
		const client = {
			session: {
				get: mock(async () => ({
					data: { directory: "/tmp", title: "Emergency" },
				})),
				prompt,
				abort,
			},
...
describe("live transform protected-token window", () => {
	async function runMassWindowFixture(options: {
		...
		const transformDeps = {
			...
			liveModelBySession: new Map([
				[
					options.sessionId,
					{ providerID: "window-provider", modelID: "window-200k" },
				],
			]),
	...
	it("protects the newest 16 one-thousand-token tools at the derived 16k floor for 200k usableSoft", async () => {
		expect(
			await runMassWindowFixture({ sessionId: "ses-derived-protected-window" }),
