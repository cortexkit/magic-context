...
| Log file (diagnostic, non-essential) | `{tmpdir}/{harness}/magic-context/magic-context.log` — `{tmpdir}` is the OS temp directory (e.g. `/tmp` on Linux/macOS), `{harness}` is `opencode` (OpenCode) or `pi` (Pi) |
